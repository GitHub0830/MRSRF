...
class weightPredictModule(nn.Module):
    def __init__(self, in_channel):
        super(weightPredictModule, self).__init__()
        self.conv_img = nn.Conv2d(in_channel, in_channel, 3, 1, 1, bias=True)
        self.relu = nn.ReLU(inplace=True)
        self.conv_flow = nn.Conv2d(2, in_channel, 3, 1, 1, bias=True)
        self.resBlock1 = ResidualBlockNoBN(num_feat=in_channel)
        self.conv_concat = nn.Conv2d(3*in_channel, in_channel, 3, 1, 1, bias=True)
        self.resBlock3 = make_layer(ResidualBlockNoBN, 3, num_feat=in_channel)
        self.conv_final = nn.Conv2d(in_channel, in_channel, 3, 1, 1, bias=True)

    def forward(self, target_fea, ref_fea, flow):
        target_fea = self.relu(self.conv_img(target_fea))
        ref_fea = self.relu(self.conv_img(ref_fea)) #ei
        res = ref_fea - target_fea

        # flow_fea = self.resBlock1(self.relu(self.conv_flow(flow%1))) #如何理解Deep Burst Super-Resolution中的modulo 1
        flow_fea = self.resBlock1(self.relu(self.conv_flow(flow)))
        
        fea_concat = torch.cat((res, ref_fea, flow_fea), dim=1)
        out = self.conv_final(self.resBlock3(self.relu(self.conv_concat(fea_concat))))
        return out
...
class pixelFusionNet(nn.Module):
    def __init__(self, Level=8, num_grad_block=5, num_feat=64, in_c=3, out_c=3):
        ...
        self.weightPredict0 = weightPredictModule(in_channel=2**4) 
        self.weightPredict1 = weightPredictModule(in_channel=(2**4 + 2**5))
        self.weightPredict = weightPredictModule(in_channel=(2**4 + 2**5 + 2**6))
    ...
    def forward(self, x1, x2):
        ...
        for i in range(0, self.level): #从coarse到fine
            ...
            ref_final_tmp = warp(ref_fea[i], flow_final)
            ...
            if i == 0:
                ref_weight = self.weightPredict0(target_fea[i], ref_final_tmp, flow_final)
            elif i == 1:
                ref_weight = self.weightPredict1(target_fea[i], ref_final_tmp, flow_final)
            else:
                ref_weight = self.weightPredict(target_fea[i], ref_final_tmp, flow_final)
            ref_weights.append(ref_weight)
        
        ## Fusion融合
        fea_cat = torch.cat((target_fea[-1], ref_final[0]*ref_weights[0]), dim=1)
        ...
        for i in range(0, self.level-1): #从coarse到fine
            ...
            fea_cat = torch.cat((target_fea[i_reverse], ref_final[i+1]*ref_weights[i+1], fea_up), dim=1)
        ...
        grad = self.grad_conv_last(fea_fusion_grad) #20220313将fea_fusion改为fea_fusion_grad
        output = self.finalConv(fea_fusion)

        return output, flows_final, delta_flows, grad #, target_fea[0], ref_final[-1], ref_weights[-1]
