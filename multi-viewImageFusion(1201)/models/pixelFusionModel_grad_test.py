...
def warp(x, flow, ind, isLast=False):
    """
    ...
    mask[mask<0.9999] = 0 #0.9999
    mask[mask>0] = 1
    
    out = output*mask
    # print(ind, 'mask.shape', mask.shape)
    # mask = torch.sum(mask, dim=1) #将通道维度的值相加
    # mask = mask.unsqueeze(dim=1)
    # mask = mask/mask.shape[1]
    # cv2.imwrite('./masks_{}.png'.format(ind), (mask.clamp(0.0, 1.0)[0].cpu().numpy().transpose(1, 2, 0) * 255.0).astype(np.uint8))
    # return output*mask
...
class pixelFusionNet(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        for i in range(0, self.level): #从coarse到fine
            i = self.level-1-i
            ref_init, mask = warp(ref_fea[i], flow_init, j)
            j += 1
            if j == 5:
                M = nn.functional.interpolate(mask, scale_factor=4, mode='bilinear', align_corners=True)
            if i == 0:
            ...
            ref_warped, mask = warp(ref_fea[i], flow_final, j)
            ref_final.append(ref_warped)
        ...
        return output, flows_final, delta_flows, grad, M
