...
class pixelFusionNet(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        for i in range(self.level):
            ...
            gradFea = self.grad_conv_1[i](imgGrad) #图像变特征
            # targetImgGradFea.append(self.grad_block_1[i](gradFea) + gradFea) #获得高层特征。这儿是不是不对？
            targetImgGradFea.append(self.grad_block_1[i](gradFea)) #改，试试20220313
        ...
        grad = self.grad_conv_last(fea_fusion_grad) #应该是fea_fusion_grad吧？改20220313
        output = self.finalConv(fea_fusion)

        return output, flows_final, delta_flows, grad
